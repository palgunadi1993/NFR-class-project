...
Permx(1:end,1)=1; Permx(frac_id,1)=10000; 
...
% function plotp_Sw_time(matrix_frac_center,matrix_pos,Swij,time)
% T = delaunay(matrix_pos(:,1),matrix_pos(:,2));
% F = scatteredInterpolant(matrix_frac_center(:,1),matrix_frac_center(:,2),Swij);
% [xq,yq] = meshgrid(meshgrid(0:2:50));
% F.Method = 'linear';
% vq2 = F(xq,yq);
% 
% colormap jet;
% triplot(T,matrix_pos(:,1),matrix_pos(:,2),'k')
% hold on
% contourf(xq,yq,vq2)
% c=colorbar;
% c.Label.String = 'Sw';
